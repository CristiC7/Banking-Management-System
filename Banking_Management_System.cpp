...
ifstream Inp;             // Global input file stream
ofstream Out;             // Global output file stream

// Function to get current date and time as formatted string
string getCurrentTime() {
	time_t now = time(0);              // Get current time
	char currentDate[11];              // Buffer for date (MM/DD/YY)
	char currentTime[9];               // Buffer for time (HH:MM:SS)
	_strdate_s(currentDate, 11);       // Get formatted date
	_strtime_s(currentTime, 9);        // Get formatted time
	return string(currentDate) + " " + string(currentTime); // Combine date and time
}
...
{
protected:
	string username;      // User identification name
	string password;      // User authentication secret
...
public:
	...
	// Pure virtual login function (must be implemented by derived classes)
...
public:
	...
	bool login()
	{
		...
		else
		{
			...
			char y;
			cin >> y;
			// Allow password change on failed login
			if (y == 'Y' || y == 'y')
			{
...
public:
	Admin() // Default constructor sets default credentials
	{
		username = "admin";
		password = "admin";
	}
	...
	void display() // Admin display method
	{
		cout << "Admin" << endl;
	}
...
protected:
	...
	string cardtype;      // Type of card (e.g., debit/credit)
	string email;         // Account holder's email
	int phonenumber;      // Account holder's phone number

public:
	Account() // Default constructor initializes empty values
	{
		accountNumber = 0;
		firstname = "";
		lastname = "";
	...
	// Accessors and mutators for account properties
	string getFirstName()
	{
		return firstname;
	}
	...
	// Pure virtual methods (interface for derived accounts)
...
{
private:
	string savedatafile;      // File name for account data storage
	int size;                 // Number of accounts
	Customer** customer;      // Array of customer pointers
	SavingsAccount** saveaccount; // Array of savings account pointers
...
public:
	...
	// Constructor that loads account data from file
	SavingsAccount(string filename, int S)
	{
		...
		while (getline(Inp, line))
		{
			...
			istringstream SS(line); // Use string stream for parsing
			// Parse account data fields
			...
			// Create Customer and SavingsAccount objects
	...
	// Parameterized constructor for individual accounts
	...
	void deposit(double amount)
	{
		...
		if (found)
		{
			// Authenticate before transaction
			if (customer[Index]->login())
			{
				saveaccount[Index]->balance += amount; // Update balance
				cout << setw(30) << "\t\tDeposit of $" << amount << " into account " << saveaccount[Index]->accountNumber << " successful." << endl;
				SavingsAccount::saveAccountData(); // Save account data
			}
		...
		if (found)
		{
			...
			if (customer[Index]->login())
			{
				...
				catch (runtime_error e)
				{
					...
					cin >> Amount_W;
					SavingsAccount::withdraw(Amount_W); // Recursive call, Retry withdrawal
				}
			}
		}
		else
	...
	{
		int updateaccount;
		SavingsAccount::display(); // Show existing accounts
		...
		if (found)
		{
			// Collect updated account information
			...
			SavingsAccount::saveAccountData(); // Save updated account data
		}
		else
		{
	...
	{
		size = S; // Set current account count
		...
		char c;
		bool added = false;
		do // Loop for multiple account additions
		{
			...
			if (added)
			{
				// Create new customer and account objects
	...
	void serachaccount()
	{
		...
		// Linear search through accounts
		for (int i = 0; i < size; i++)
		{
	...
	{
		int deleteacc;
		if (admin.login()) // Require admin authentication
		{
			SavingsAccount::display(); // Display all accounts
			...
			if (found)
			{
				...
				if (a == 'Y' || a == 'y')
				{
					// Validate index before deletion account from file
					if (deleteIndex >= 0 && deleteIndex < size) {
						...
						size--; // Decrement account count
						SavingsAccount::saveAccountData(); // Save updated data to file
	...
	{
		Out.open(savedatafile, ios::out); // Overwrite mode
		...
		// Write student to file, all records
		for (int i = 0; i < size; i++)
		{
			cust = customer[i];
	...
	~SavingsAccount() // Destructor for memory cleanup
	{
		SavingsAccount::saveAccountData(); // Save data to file before destruction
		if (saveaccount != nullptr)
		{
			delete[] saveaccount; // Deallocate memory for saveaccount array(Free account array)
		}
		if (customer != nullptr) {
			delete[] customer; // Deallocate memory for customer array(Free customer array)
		}
	}
};
...
{
private:
	string checkdatafile;   // Data storage filename
	int size;               // Account count
	Customer** customer;    // Customer pointers array
	CheckingAccount** checkaccount; // Account pointers array
	Admin admin;            // Admin instance
	double feecharge;       // Transaction fee
public:
	...
	CheckingAccount(string filename, int size) 
	{
		...
		this->size = size;
		this->feecharge = 0.5; // Set transaction fee
		// Allocate memory
		customer = new Customer * [size];
	...
	CheckingAccount(string firstname, string lastname, int accountNumber, double balance, string cardtype, string email, int phonenumber)
		: Account(firstname, lastname, accountNumber, balance, cardtype, email, phonenumber) {
		this->feecharge = 0.5; // Set default fee
	...
	void deposit(double amount)
	{
		...
		if (found)
		{
			...
			if (customer[Index]->login())
			{
				amount -= feecharge; // Deduct fee
				checkaccount[Index]->balance += amount; // Update balance
				...
				CheckingAccount::saveAccountData(); // Save data to file
			}

		}
	}

	// Withdrawal with fee deduction
	void withdraw(double amount)
	{
		...
		if (found)
		{
			...
			if (customer[Index]->login())
			{
				try
				{
					...
					{
						checkaccount[Index]->balance -= amount; // Withdraw amount
						checkaccount[Index]->balance -= feecharge; // Deduct fee
						cout << "\t\t\t\t\tFee charges $" << feecharge << " dedicated from your Balance\n";
						cout << "\t\t\t\Withdrawal of $" << amount << " from account " << checkaccount[Index]->accountNumber << " successful at " << getCurrentTime() << endl;
						CheckingAccount::saveAccountData(); // Save changes
					}
					else
					{
						throw runtime_error("\t\t\tInsufficient balance in account ");
				...
				catch (runtime_error e)
				{
					...
					cout << "\t\t\tEnter the Amount you want to withdraw" << endl;
					cin >> Amount_W;
					CheckingAccount::withdraw(Amount_W); // Retry withdrawal
				}
			}
		}
		else {
	...
	{
		int updateaccount;
		CheckingAccount::display(); // Display accounts
		...
		if (found)
		{
			...
			if (added)
			{
				...
				CheckingAccount::saveAccountData(); // Save updated account data to file
			}
		}
		else
		{
	...
	// Add new checking account functionality
	void Addaccount(int s)
	{
		Out.open(checkdatafile, ios::app); // Open file in append mode
		if (!Out)
		{
			cout << left << setw(30) << "File is not open!" << endl;
			return;
		...
		char c;
		bool added;
		do // Loop for multiple account creation
		{
	...
	void deleteaccount(string filename, int s)
	{
		...
		if (admin.login()) // Require admin authentication
		{
			CheckingAccount::display(); // Show all accounts
			...
			if (found)
			{
				...
				cin >> y;
				if (y == 'Y' || y == 'y') {
					// Delete account and customer objects
					delete customer[deleteIndex];
					...
					size--; // Decrement account count
					CheckingAccount::saveAccountData(); // Save updated account data
	...
	void saveAccountData()
	{
		Out.open(checkdatafile, ios::out); // Overwrite mode
	...
	~CheckingAccount()
	{
		CheckingAccount::saveAccountData(); // Save account data before destruction
		if (checkaccount != nullptr)
		{
			delete[] checkaccount; // Free account array
		}
		if (customer != nullptr) 
		{
			delete[] customer; // Free customer array
		}
	}
};
...
{
protected:
	int loanNumber;         // Unique loan identifier
	double loanAmount;      // Principal loan amount
	double interestRate;    // Annual interest rate
	string email;           // Borrower's email
	string phonenumber;     // Borrower's phone number
	string firstname;       // Borrower's first name
	string lastname;        // Borrower's last name
...
// Parameterized constructor
	...
	// Pure virtual methods (loan operations)
	...
	virtual void updateaccount() = 0;
	virtual void deleteAccount(string filename, int size) = 0;
	virtual ~Loan() {} // Virtual destructor
};

// Student Fee Loan specialization
class StudentFeeLoan : public Loan
{
private:
	string enrollmentYear;      // Academic year of enrollment
	string studentLoanDataFile; // Data storage filename
	int size;                  // Loan count

	Customer** customer;       // Customer pointers array
	Admin admin;               // Admin instance
	StudentFeeLoan** studentLoans; // Loan objects array
...
// Default constructor
	...
	StudentFeeLoan(string filename, int size)
	{
		...
		while (getline(Inp, line))
		{
			...
			// Create a new StudentFeeLoan object
	...
	{
		this->enrollmentYear = year; // Set enrollment year
	}

	// Accessor for enrollment year
	...
	// Add new loan functionality
	void addloan()
	{
		Out.open(studentLoanDataFile, ios::app); // Open the file(Append mode)
		if (!Out)
		{
		...
		char c;
		bool added;
		do // Loop for multiple loan creation
		{
			...
			if (added)
			{
				...
				size++; // Increment loan count

				// Write new loan to file
				for (int i = size - 1; i < size; i++)
				{
					Customer* cust = customer[i];
					StudentFeeLoan* loan = studentLoans[i];
	...
	// Update loan account functionality
	void updateaccount()
	{
		int updateloan;
		StudentFeeLoan::display(); // Display existing loans
		...
		if (found)
		{
			// Collect updated loan information
			...
			if (updated)
			{
				// Update loan and customer objects
				...
				StudentFeeLoan::saveLoanData(); // Save updated loan data to file
			}
			else
			{
	...
	// Delete loan account functionality
	void deleteAccount(string filename, int s)
	{
		int deleteLoanNumber;
		if (admin.login()) // Require admin authentication
		{
			StudentFeeLoan::display(); // Display all loan accounts
			...
			if (found)
			{
				...
				cin >> y;
				if (y == 'Y' || y == 'y') {
					// Delete objects and shift array
					delete customer[deleteIndex];
	...
	// Display all loans in tabular format
	void display()
	{
	...
	~StudentFeeLoan()
	{
		StudentFeeLoan::saveLoanData(); // Save data before destruction
		if (studentLoans != nullptr)
		{
			delete[] studentLoans; // Free loan array
		}
		if (customer != nullptr)
		{
			delete[] customer; // Free customer array
...
// Personal Loan specialization
class Personalloan : public Loan
{
private:
	int loantime; // Loan duration in years
	string personLoanDataFile; // Data storage filename
	int size; // Loan count

	Customer** customer; // Customer pointers array
	Admin admin; // Admin instance
	Personalloan** personLoans; // Loan objects array
...
// Default constructor
	...
	Personalloan(string firstname, string lastname, int lnNumber, double amount, double rate, int loantime, string email, string phonenumber)
		: Loan(firstname, lastname, lnNumber, amount, rate, email, phonenumber)
	{
		this->loantime = loantime; // Set loan duration
	}

	// Accessors and mutators for loan duration
	void settime(int tm)
	{
		loantime = tm;
	}
	...
	// Calculate interest for a personal loan
	void calculateInterest()
	{
	...
	// Update personal loan account functionality
	void updateaccount()
	{
		bool updated = false;
		int updateloan;
		Personalloan::display(); // Show existing loans
		...
		if (found)
		{
			...
			if (updated)
			{
				...
				cout << "\t\tAccount Edited successfully at " << getCurrentTime() << endl;
				Personalloan::saveLoanData(); // Save updated loan data
			}
			else
			{
	...
	// Display all personal loans in tabular format
	void display()
	{
	...
	// Search personal loan functionality
	void sreachaccount()
	{
		int serachaccount;
		cout << "\t\t\tEnter the Loan Number to serach: ";
	...
	// Save all personal loans to file
	void saveLoanData()
	{
	...
	// Delete personal loan account functionality
	void deleteAccount(string filename, int s)
	{
		int deleteacc;
		if (admin.login()) // Require admin authentication
		{
			Personalloan::display(); // Display all personal loans
			cout << "\t\t\tEnter the loan number you want to delete: ";
			cin >> deleteacc;
			bool found = false;
			int deleteIndex = -1;
	...
	~Personalloan()
	{
		Personalloan::saveLoanData(); // Save data before destruction
		if (personLoans != nullptr)
		{
			delete[] personLoans; // Free loan array
		}
		if (customer != nullptr)
		{
			delete[] customer; // Free customer array
...
{
protected:
	int investmentID;          // Unique investment identifier
	string firstname;         // Investor's first name
	string lastname;          // Investor's last name
	double investmentAmount;  // Investment principal
	string investmentDataFile; // Data storage filename
	string email;             // Investor's email
	string phonenumber;       // Investor's phone number
...
// Accessors and mutators
	...
	// Pure virtual methods (investment operations)
...
// Relative Investment specialization
class RelativeInvestment : public Investment
{
private:
	string risklevel;          // Investment risk category
	string relativeName;       // Name of relative involved
	string relativeinvestFile; // Data storage filename
	int size;                 // Investment count
	Admin admin;              // Admin instance

	Customer** customer;      // Customer pointers array
	RelativeInvestment** investments; // Investment objects array
...
// Default constructor
	...
	// Accessors and mutators for risk and relative
	...
	// Accessor for relative name
	string getrelativename()
	{
		return relativeName;
	}

	// Delete investment account functionality
	void deleteAccount(string filename, int s)
	{
		int deleteacc;
		if (admin.login()) // Check if admin is logged in
		{
			RelativeInvestment::display(); // Display all investments
			...
			if (found)
			{
				...
				if (y == 'Y' || y == 'y') {
					...
					size--; // Decrement investment count
					RelativeInvestment::saveData(); // Save data
	...
	void saveData()
	{
		Out.open(relativeinvestFile); // Open file for writing

		if (Out.is_open())
		{
			// Write all investment records to file
			for (int i = 0; i < size; i++)
			{
				Customer* cust = customer[i];
	...
	// Accessor for risk level
	string getrisklevel()
	{
		return risklevel;
	}

	// Search investment functionality
	void sreachaccount()
	{
		int serachaccount;
		cout << "\t\tEnter the Loan Number to serach: ";
		...
		// Display investment details if found
		if (found1)
		{
	...
	void addinvest()
	{
		...
		do // Loop for multiple investments
		{
			...
			if (added)
			{
				// Create new investment and customer objects
				Customer* cust = new Customer(us, pass);
				...
				size++; // Increment investment count
			}
	...
	// Update investment account functionality
	void updateaccount()
	{
		bool updated = false;
		int updateinvest;
		RelativeInvestment::display(); // Display investments
		cout << "\t\t\tEnter the loan number to update: ";
		cin >> updateinvest;
		...
		// Find investment to update
		for (int i = 0; i < size; i++)
		{
			if (investments[i]->getinvestmentID() == updateinvest)
			{
				// Display current investment details
		...
		if (found)
		{
			// Collect updated investment information
			cout << "\t\t\tEnter first name: ";
			cin >> fname;
			cout << "\t\t\tEnter last name: ";
			cin >> lname;
			...
			if (updated)
			{
				...
				RelativeInvestment::saveData(); // Save changes
			}
			else
			{
	...
	// Display all investments in tabular format
	void display()
	{
	...
	// Destructor for memory cleanup
	~RelativeInvestment() {
		RelativeInvestment::saveData(); // // Save data before destruction
		if (customer != nullptr) {
			delete[]customer; // Deallocate memory for customer array
		}
		if (investments != nullptr)
		{
			delete[]investments; // Deallocate memory for investments array
		}
	}
};
...
void aboutus()
{
	...
	// Project description
	cout << "\t\t\t\t\tIt is a Simple Bank Management System Project ";
	cout << " based on the OOP C++ programming language.";
...
int main()
{
	...
	// Count lines in data files to determine initial sizes
	int S_Size = 0, C_Size = 0, St_Size = 0, P_Size = 0, R_Size = 0;
	S_Size = CountLinesInFile("save.txt");        // Savings accounts
	C_Size = CountLinesInFile("check.txt");       // Checking accounts
	St_Size = CountLinesInFile("student.txt");    // Student loans
	P_Size = CountLinesInFile("personal.txt");    // Personal loans
	R_Size = CountLinesInFile("invest.txt");      // Investments

	// Initialize system components with data files
	...
	case 1: // Administrator portal
		...
		while (AloggedIn)
		{
			...
			switch (adchoice)
			{
			case 1: // Savings account management
				system("cls"); 
				// Savings account admin menu
				cout << "\n\n";
				cout << "\t\t\t\t\t\t==================================================" << endl;
				cout << "\t\t\t\t\t\t\t\tSaving Account Admin Potral\n";
				cout << "\t\t\t\t\t\t==================================================" << endl;

				// Savings account admin options
				cout << "\n\t\t\t\t\t\tSelect one Option\n";
				...
				case 1: // View customer accounts
					...
					cin >> x;
					break;
				case 2: // Customer account registration
					system("cls");
				...
				case 4: // Delete customer account
					...
					cin >> x;
					break;
				case 5: // Search customer account
					system("cls");
					SA->serachaccount();
					cout << "\t\t\t\t\t\tEnter any Character to goto Main menu...";
					cin >> x;
				...
				break;
			case 2: // Checking account management
				system("cls");
				// Checking account admin menu
				cout << "\n\n";
				cout << "\t\t\t\t\t\t==================================================" << endl;
				cout << "\t\t\t\t\t\t\tChecking Account Admin Potral" << endl;
				cout << "\t\t\t\t\t\t==================================================" << endl;

				// Checking account admin options
				cout << "\n\n\t\t\t\t\t\t[1] . View Customer Accounts\n";
				cout << "\t\t\t\t\t\t[2] . Customer Account Registration\n";
				cout << "\t\t\t\t\t\t[3] . Edit Customer Account\n";
				cout << "\t\t\t\t\t\t[4] . Delete Customer Account\n";
				...
				break;
			case 3: // Student loan management
				system("cls");
				// Student loan admin menu
				cout << "\n\n";
				cout << "\t\t\t\t\t\t==================================================" << endl;
				cout << "\t\t\t\t\t\t\t\tStudent Loan Admin Potral" << endl;
				cout << "\t\t\t\t\t\t==================================================" << endl;

				// Student loan admin options
				cout << "\n\t\t\t\t\t\tSelect one Option\n\n";
				...
				case 1: // View student accounts
					...
					cin >> x;
					break;
				case 2: // Student account registration
					system("cls");
				...
				break;
			case 4: // Personal loan management
				system("cls");
				// Personal loan admin menu
				cout << "\n\n";
				cout << "\t\t\t\t\t\t==================================================" << endl;
				cout << "\t\t\t\t\t\t\t\tPersonal Loan Admin Potral" << endl;
				cout << "\t\t\t\t\t\t==================================================\n" << endl;

				// Personal loan admin options
				...
				case 1: // View personal accounts
					...
					cin >> x;
					break;
				case 2: // Personal account registration
					system("cls");
					...
					cin >> x;
					break;
				case 3: // Edit personal account
					system("cls");
					PL->updateaccount();
					cout << "\t\t\t\t\t\tEnter any Character to goto Main menu...";
					cin >> x;
					break;
				case 4: // Delete personal account
					system("cls");
					PL->deleteAccount("personal.txt", P_Size);
					cout << "\t\t\t\t\t\tEnter any Character to goto Main menu...";
					cin >> x;
					break;
				case 5: // Search personal account
					system("cls");
					PL->sreachaccount();
					cout << "\t\t\t\t\t\tEnter any Character to goto Main menu...";
					cin >> x;
				...
				break;
			case 5: // Relative investment management
				system("cls");
				// Relative investment admin menu
				cout << "\n\n";
				cout << "\t\t\t\t\t\t==================================================" << endl;
				cout << "\t\t\t\t\t\t\tRelative Investment Admin Potral" << endl;
				cout << "\t\t\t\t\t\t==================================================\n" << endl;

				// Relative investment admin options
				...
				{
				case 1: // View relative investment accounts
					system("cls");
					// Display relative investment accounts
					RI->display();
					cout << "\t\t\t\t\t\tEnter any Character to goto Main menu...";
					cin >> x;
					break;
				case 2: // Relative investment account registration
					system("cls");
					// Add new relative investment account
					RI->addinvest();
					cout << "\t\t\t\t\t\tEnter any Character to goto  Main menu...";
					cin >> x;
					break;
				case 3: // Edit relative investment account
					system("cls");
					RI->updateaccount();
					cout << "\t\t\t\t\t\tEnter any Character to goto  Main menu...";
					cin >> x;
					break;
				case 4: // Delete relative investment account
					system("cls");
					RI->deleteAccount("invest.txt", R_Size);
					cout << "\t\t\t\t\t\tEnter any Character to goto  Main menu...";
					cin >> x;
					break;
				case 5: // Search relative investment account
					system("cls");
					RI->sreachaccount();
					cout << "\t\t\t\t\t\tEnter any Character to goto  Main menu...";
					cin >> x;
	...
	case 2: // User
		...
		while (UloggedIn)
		{
			...
			case 3: // Student loan
				...
				case 1: // Loan Inquiry
					system("cls");
					// Display student loan balance
					S_L->sreachaccount();
					cout << "\t\t\t\t\t\tEnter any Character to goto  Main menu...";
					cin >> x;
					break;
				case 2: // Calculate Intrest
					system("cls");
			...
			case 4: // Personal loan
				...
				break;
			case 5: // Relative Investment
				system("cls");
				// Relative Investment menu
				cout << "\n\n";
				cout << "\t\t\t\t\t\t==================================================" << endl;
				cout << "\t\t\t\t\t\t\t\tRelative Investment Potral" << endl;
				cout << "\t\t\t\t\t\t==================================================" << endl;

				// Relative Investment options
				...
				switch (invectchoice)
				{
				case 1: // Investment Inquiry
					system("cls");
					// Display relative investment balance
					RI->sreachaccount();
					cout << "\t\t\t\t\t\tEnter any Character to goto Main menu...";
					cin >> x;
					break;
				case 2: // Calculate Intrest
					system("cls");
					// Calculate relative investment returns
					RI->calculateReturns();
					cout << "\t\t\t\t\t\tEnter any Character to goto Main menu...";
					cin >> x;
					break;
	...
	// Deallocate memory
	delete SA; // Savings Account
	delete CA; // Checking Account
	delete S_L; // Student Loan
	delete PL; // Personal Loan
	delete RI; // Relative Investment
	return 0;
}